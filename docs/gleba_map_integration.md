...
6. **Erodibility K_sp = `SoilDepth`** — a clean continuous viridis ramp where bright = deep soil = thick erodible regolith. `K_eff = K_base × contrast^(2·norm−1)` (norm 0.5 = neutral) via the **Erodibility Contrast** knob, decoded by luminance (direction verified: brighter→higher K). Detect auto-fills it; works with any continuous softness map. The categorical `RockType` map is intentionally NOT used for erodibility (ordering hardness per class needs a legend that isn't available); the soil map replaces it.
